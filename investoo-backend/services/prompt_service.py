def generate_response(query, context):
    ...
    if "portfolio" in query:
        if not portfolio_companies:
            return "Your portfolio is currently empty. Add some companies to get personalized insights!"
        return f"Your portfolio contains {len(portfolio_companies)} companies: {', '.join(portfolio_companies)}."

    # 🔥 SPECIFIC COMPANY QUERIES (handle first)
    if company:
        # WHY IN PORTFOLIO
        if "why" in query and company in portfolio_companies:
            if prediction > 0.8:
                return (
        ...
        if "why not" in query and company not in portfolio_companies:
            if prediction > 0.8:
                return (
    ...
    # GENERAL PORTFOLIO-BASED RESPONSES (handle last)
    if "portfolio" in query and not company:
        if not portfolio_companies:
            return (
